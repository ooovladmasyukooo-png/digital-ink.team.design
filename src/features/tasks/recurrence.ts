...
export function defaultRecurrenceRule(kind: TaskRecurrenceKind, deadlineIso: string | null): TaskRecurrenceRule {
...
export function normalizeRecurrenceRule(
  rule: TaskRecurrenceRule | LegacyTaskRecurrenceRule | null | undefined,
  ...
  const legacy = rule as LegacyTaskRecurrenceRule;
  switch (legacy) {
    case 'daily':
...
function sanitizeRecurrenceRule(rule: TaskRecurrenceRule): TaskRecurrenceRule {
  switch (rule.kind) {
    case 'daily': {
...
function matchesRule(date: Date, rule: TaskRecurrenceRule, seriesAnchor: Date): boolean {
  switch (rule.kind) {
    case 'daily':
...
function resetSubtasksForRecurrence(subtasks: TaskSubtask[]): TaskSubtask[] {
...
export function spawnRecurringTask(task: Task, newId: string, now = new Date()): Task {
  if (!task.recurrenceRule) return task;
  const rule = sanitizeRecurrenceRule(task.recurrenceRule);

  return {
    ...task,
    id: newId,
    status: 'new',
    deadline: nextDeadlineAfter(task.deadline, rule, now),
    completedAt: null,
...
export function shouldSpawnRecurring(prev: Task, patch: { status?: Task['status'] }): boolean {
