...
export function TaskPickerPopover({
  ...
  return createPortal(
    ...
    >
      {searchable ? (
        <div className={styles['ts-picker-search']}>
          <div className={styles['ts-picker-search-in']}>
            <span className={styles['ts-picker-search-i']}>{Icons.search}</span>
            <input
              type="search"
              placeholder="Пошук…"
              value={query}
      ...
      {children}
      <div className={cx(styles['ts-picker-list'], compact && styles['ts-picker-list-compact'])}>
        {filtered.length === 0 ? (
          <div className={styles['ts-picker-empty']}>Нічого не знайдено</div>
        ) : (
          filtered.map((item) => (
            <button
              key={item.id}
      ...
      {clearOption ? (
        <div className={styles['ts-picker-foot']}>
          <button
            type="button"
