...
export function DangerZone({ projectName, onConfirmDelete }: DangerZoneProps) {
    ...
    <section className={styles['td-sect']}>
      <div className={cx(styles['acc-row'], styles['danger-row'], awaitingConfirm && styles['danger-row-active'])}>
        <div className={styles['acc-l']}>
          <div className={styles['acc-k']}>{awaitingConfirm ? 'Підтвердіть видалення' : 'Видалити проєкт'}</div>
          <div className={styles['acc-v']}>
            {awaitingConfirm ? (
              <>
                Буде видалено профіль <strong className={styles['danger-name']}>{projectName}</strong>. Цю дію не можна скасувати.
              </>
            ) : (
